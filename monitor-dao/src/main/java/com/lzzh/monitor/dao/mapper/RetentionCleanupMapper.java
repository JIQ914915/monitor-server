...
public interface RetentionCleanupMapper {
    ...
    @Delete("DELETE FROM pg_operational_snapshot WHERE NOT (collected_at >= now() - make_interval(days => #{retentionDays}))")
    int deletePgOperationalSnapshotsOlderThanDays(@Param("retentionDays") int retentionDays);
