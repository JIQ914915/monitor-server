...
/** 天级采集任务（分片广播；采集容量趋势、库对象统计等低频指标，§14.1）。 */
@Component
public class DailyCollectJobHandler {

    private final InstanceService instanceService;
    private final CollectRunner collectRunner;

    public DailyCollectJobHandler(InstanceService instanceService, CollectRunner collectRunner) {
        this.instanceService = instanceService;
        this.collectRunner = collectRunner;
    }

    @XxlJob("dailyCollectJobHandler")
    public void dailyCollect() {
        ...
        XxlJobHelper.log("[天级] 分片 {}/{} 待采集实例数: {}", idx, total, shard.size());
        collectRunner.run(shard, CollectFrequency.DAILY);
    }
}
