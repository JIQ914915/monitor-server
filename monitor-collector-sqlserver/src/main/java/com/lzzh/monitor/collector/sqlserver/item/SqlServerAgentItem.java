...
@Component public class SqlServerAgentItem implements SqlServerMetricItem{
 ...
 public void collect(Connection c,CollectRequest r,SqlServerVersionAdapter a,SqlServerMetricSink s)throws Exception{if(expressEdition(c))return;long ts=System.currentTimeMillis();try(Statement st=c.createStatement()){st.setQueryTimeout(10);try(ResultSet x=st.executeQuery(a.agentHealthSql())){if(!x.next())return;emit(x,s,"sqlserver.agent.job_count","job_count",ts);emit(x,s,"sqlserver.agent.disabled_jobs","disabled_jobs",ts);emit(x,s,"sqlserver.agent.failed_jobs","failed_jobs",ts);emit(x,s,"sqlserver.agent.running_jobs","running_jobs",ts);}}}
 private static boolean expressEdition(Connection c)throws SQLException{try(Statement s=c.createStatement();ResultSet r=s.executeQuery("SELECT CAST(SERVERPROPERTY('EngineEdition') AS int)")){return r.next()&&r.getInt(1)==4;}}
